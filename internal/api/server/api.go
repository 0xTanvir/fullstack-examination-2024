...
import (
	...
	"github.com/zuu-development/fullstack-examination-2024/internal/api/handler"
...
func NewAPI(opts TodoAPIServerOpts) (Server, error) {
	...
	dbInstance, err := sqlite.New(opts.Config.SQLite.DBFilename)
	if err != nil {
...
func (s *todoAPIServer) Run() error {
	log.Infof("%s %s serving on port %d", s.Name(), version.Get(), s.port)
...
func (s *todoAPIServer) Shutdown(ctx context.Context) error {
	log.Infof("shuting down %s %s serving on port %d", s.Name(), version.Get(), s.port)
