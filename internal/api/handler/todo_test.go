...
func TestTodoHandler_Create(t *testing.T) {
	...
	dbInstance, err := sqlite.NewMemory()
	require.NoError(t, err)
	err = sqlite.Migrate(dbInstance)
	require.NoError(t, err)
	repository := sqlite.NewTodo(dbInstance)
	...
	tests := []struct {
	...
	}{
		{
			...
			createBody: `{"task":"Created Task1","priority":"low"}`,
			want: want{
				StatusCode: http.StatusCreated,
				Response:   []byte(`{"data":{"id":1,"task":"Created Task1","status":"created","priority":"low"}}`),
			},
		},
		{
			name:       "successful_create_but_with_ignore_status",
			createBody: `{"task":"Created Task2", "priority":"low", "status":"done"}`,
			want: want{
				StatusCode: http.StatusCreated,
				Response:   []byte(`{"data":{"id":2,"task":"Created Task2","status":"created","priority":"low"}}`),
			},
		},
		{
...
func TestTodoHandler_Update(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			createBody: `{"task":"Created Task","priority":"low"}`,
			updateBody: `{"task":"Updated Task","status":"done","priority":"high"}`,
			want: want{
				StatusCode: http.StatusOK,
				Response:   []byte(`{"data":{"id":3,"task":"Updated Task","status":"done","priority":"high"}}`),
			},
		},
		{
...
func TestTodoHandler_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			createBody: `{"task":"Deleted Task","priority":"low"}`,
			want: want{
...
func TestTodoHandler_Find(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			createBody: `{"task":"Found Task","priority":"low"}`,
			want: want{
				StatusCode: http.StatusOK,
				Response:   []byte(`{"data":{"id":5,"task":"Found Task","status":"created","priority":"low"}}`),
			},
		},
		{
			name:   "not_found",
...
func createTask(t *testing.T, e *echo.Echo, handler definition.TodoHandler, body string) int {
